...
describe('resource invalidation registry', () => {
  ...
  it.each(['notes', 'folders', 'activity', 'chat', 'people'] as const)('accepts and invalidates the new %s resource family', async (resource) => {
    const client = new QueryClient()
    const accountID = 'account-a'
    ...
    client.setQueryData(queryKeys.people(accountID), {})
    await invalidateResource(client, accountID, resource)
